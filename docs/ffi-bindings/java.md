...
```bash
java -jar cobble-0.4.0.jar --help
java -jar cobble-0.4.0.jar remote-compactor --config ./config.yaml --bind 127.0.0.1:18888
java -jar cobble-0.4.0.jar web-monitor --config ./config.yaml --bind 127.0.0.1:8080
```
...
Java native build commands use `--features storage-all` on `cobble-java`, so Java artifacts include all Cobble optional OpenDAL backend features (`alluxio`, `cos`, `oss`, `s3`, `ftp`, `goosefs`, `hdfs`, `sftp`).
