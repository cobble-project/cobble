...
if [[ "${SKIP_BUILD}" -eq 0 ]]; then
  TMP_CONFIG="$(mktemp "${TMPDIR:-/tmp}/cobble-docs-config.XXXXXX.yml")"
  trap 'rm -f "${TMP_CONFIG}"' EXIT
  cat > "${TMP_CONFIG}" <<EOF
docs_root_label: ${ROOT}
docs_commit_label: ${COMMIT_SHORT}
EOF
  (
    cd "${DOCS_DIR}"
    bundle exec jekyll build --strict_front_matter --baseurl "${BASEURL}" --config "_config.yml,${TMP_CONFIG}"
  )
fi
...
TMP_DIR="$(mktemp -d)"
if [[ "${SKIP_BUILD}" -eq 0 ]]; then
  trap 'rm -f "${TMP_CONFIG}"; rm -rf "${TMP_DIR}"' EXIT
else
