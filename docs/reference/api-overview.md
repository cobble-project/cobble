...
`cobble-table` provides typed schemas, rows, and shard/global readers over the core storage engine.
Use `.current_global_snapshot()` for a reader that follows committed snapshots, or
`.global_snapshot(id)` for a fixed view.

See [Table](../getting-started/table) for schemas, reader/writer usage, Catalog configuration,
schema evolution, global snapshots, and distributed write/scan plans.
