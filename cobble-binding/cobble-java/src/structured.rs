...
impl StructuredPriorityQueueHandle {
    ...
    fn offer<K, V>(&self, bucket: u16, key: K, value: V) -> cobble_binding::Result<()>
    where
        K: AsRef<[u8]>,
        V: AsRef<[u8]>,
    {
    ...
    fn delete<K>(&self, bucket: u16, key: K) -> cobble_binding::Result<()>
    where
        K: AsRef<[u8]>,
    {
    ...
    fn advance_to<K>(&self, bucket: u16, key: K) -> cobble_binding::Result<()>
    where
        K: AsRef<[u8]>,
    {
    ...
    fn cursor(&self, bucket: u16) -> cobble_binding::Result<Option<Vec<u8>>> {
    ...
    ) -> cobble_binding::Result<Vec<(Bytes, Bytes)>> {
...
pub extern "system" fn Java_io_cobble_structured_Db_createSchemaBuilder(
    ...
    let builder = unsafe {
        ...
        std::mem::transmute::<
            cobble_binding::structured::StructuredSchemaBuilder<'_, DataStructureDb>,
            cobble_binding::structured::StructuredSchemaBuilder<'static, DataStructureDb>,
        >(builder)
    };
...
fn restore_structured_db(
    ...
    let resolver = cobble_binding::structured::structured_merge_operator_resolver();
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_io_cobble_structured_Db_cancelSnapshot(
    mut env: JNIEnv,
    _class: JClass,
    handle: jlong,
    snapshot_id: jlong,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_io_cobble_structured_Db_loadReadonlyFilesToPrimary(
    mut env: JNIEnv,
    _class: JClass,
    handle: jlong,
) -> jlong {
...
impl StructuredScanCursorHandle {
    ...
    fn next_row_direct<'local>(
        ...
        let encoded = match self.consume_next_row_with_bucket(|bucket, key, cols| {
            write_direct_scan_row_payload(env, io_addr, io_capacity, bucket, key.as_ref(), cols)
                .map_err(cobble_binding::Error::IoError)
        }) {
            Ok(row) => row,
            Err(err) => {
                throw_illegal_state(env, err.to_string());
    ...
    fn consume_next_row_with_bucket<T, F>(
        &mut self,
        mut consumer: F,
    ) -> cobble_binding::Result<Option<T>>
    where
        F: FnMut(u16, &Bytes, &[Option<StructuredColumnValue>]) -> cobble_binding::Result<T>,
    {
        match &mut self.iter {
...
pub extern "system" fn Java_io_cobble_structured_StructuredSchemaBuilder_nativeAddListColumn(
    ...
    let retain_mode = match retain_mode.as_str() {
        "first" => cobble_binding::structured::ListRetainMode::First,
        "last" => cobble_binding::structured::ListRetainMode::Last,
        _ => {
            throw_illegal_argument(
                &mut env,
                format!(
    ...
    let config = cobble_binding::structured::ListConfig {
        max_elements,
        retain_mode,
...
pub extern "system" fn Java_io_cobble_structured_StructuredSchemaBuilder_nativeSetColumnFamilyOptions(
    ...
    let options = match serde_json::from_str::<cobble_binding::ColumnFamilyOptions>(&options_json) {
        Ok(v) => v,
        Err(err) => {
            throw_illegal_argument(
                &mut env,
